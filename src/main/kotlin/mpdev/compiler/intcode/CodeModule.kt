...
interface CodeModule {
    ...
    var code: MutableList<Any>
    val labelsMap: MutableMap<String,Int>

    /** send the code to output */
    fun writeCode() {
        processLabels()
        outStream.print(code.joinToString(",","",","))
    }

    fun processLabels() {
        code.indices.forEach {
            if (code[it] is String)
                code[it] = labelsMap[code[it]]!!   // replace any labels with actual value of PC for that position
        }
    }

    /** produce code */
    fun outputCode(cmd: List<Any>) {
        code.addAll(cmd)
        PC += cmd.size
    }
    ...
    fun allocateStackVar(size: Int): Int = 0
